...
parameter_name = 'spike_rate'; % 'membrane_potential', 'membrane_potential_smoothed', 'spike_events', 'spike_rate', '*_speed', '*_velocity'
    ...
    for trial = 1:n_trials
        ...
        for transition = 1:numel(transition_indices_treadmill)
            ...
            if strcmp(parameter_source,'ephys') && ~strcmp(parameter_name,'spike_events')
                parameter(:,transition) = parameter(:,transition) - mean(parameter(1:pre_window*sampling_rate_ephys,transition));
            elseif strcmp(parameter_source,'treadmill')
                parameter(:,transition) = parameter(:,transition) - mean(parameter(1:pre_window*sampling_rate_treadmill,transition));
            end
        end
